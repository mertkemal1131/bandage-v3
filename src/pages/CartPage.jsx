...
export default function CartPage() {
  ...
  const update = (id, qty) => dispatch({ type: 'UPDATE_QTY', payload: { id, qty } });
  const remove = (id, name) => { dispatch({ type: 'REMOVE_FROM_CART', payload: id }); toast.info(`${name} removed`); };
          ...
          <div className="flex gap-6 items-start flex-wrap">
            ...
            <div className="flex-1 min-w-[320px]">
                ...
                {items.map(item => (
                  <div key={item.id} className="grid grid-cols-[3fr_1fr_1.5fr_1fr_40px] py-4 px-5 border-b border-[#E8E8E8] items-center">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-[14px] text-[#252B42] mb-[3px]">{item.name}</p>
                    ...
                    <div className="flex border border-[#E8E8E8] rounded w-[100px] overflow-hidden">
                      ...
                      <span className="w-9 flex items-center justify-center font-bold text-[14px] border-x border-[#E8E8E8]">{item.qty}</span>
                      <button onClick={() => update(item.id, item.qty + 1)} className="w-8 h-9 border-none bg-white cursor-pointer flex items-center justify-center">
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <button
                  onClick={() => { dispatch({ type: 'CLEAR_CART' }); toast.info('Cart cleared'); }}
            ...
            <div className="w-[300px] shrink-0 bg-white border border-[#E8E8E8] rounded-[5px] p-6 sticky top-[90px]">
              ...
              <div className="text-center text-[12px] text-[#737373] font-semibold">
                🔒 Secure checkout
              </div>
